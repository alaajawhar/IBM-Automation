...
var fs = require("fs");
let path="C:\\Users\\E195\\Desktop\\STC\\IBM Automated\\Generators\\addedHeadersSTCwebApp.json";
try {
    ...
    if (AllDefinitions[field]["properties"]) {
      for (prop in AllDefinitions[field]["properties"]) {
        if (
          ...
          arr = AllDefinitions[field]["properties"][prop]["items"][
            "$ref"
          ].split("/");
          if (!searchitem.includes(arr[arr.length - 1]))
            searchitem.push(arr[arr.length - 1]);
...
class swagger {
  ...
  static create() {
    ...
    fs.writeFile(
      `.\\Generators\\Useful_RestRequestApis.json`,
      ...
      function (err) {
        ...
        else console.log(`Useful_RestRequestApis.json file was successfully created`);
      }
    );
  }
}
...
swagger.add("/Mss/v1/BeginKYCVerify", "post");
swagger.add("/Global/v2/Region", "get");
swagger.add("/Global/v2/City", "post");
swagger.add("/Global/v2/District", "post");
swagger.add("/Mss/v1/FinishKYCVerify", "post");
swagger.create();